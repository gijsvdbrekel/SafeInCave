...
def plot_dilatancy_boundaries_all(ax,
                                 p_min=0.01, p_max=40.0, npts=500,
                                 show_legend=True):
    """
    Plot selected dilatancy boundaries in p–q space (MPa):
      ...
      - De Vries 2005 (ONE parameter set; comp & ext branches)

    Hunsche is intentionally left out (it dominates the axis scale unless you
    make additional unit assumptions).
    """
    ...
    for D, lab, ls in [
        ...
        sqrtJ2 = D * I1
        ax.plot(p, q_from_sqrtJ2(sqrtJ2),
                linestyle=ls, linewidth=1.3, alpha=0.9, label=lab)
    ...
    # ------------------
    # De Vries (2005) — single parameter set (your original)
    # ------------------
    ...
    psi_c = -np.pi / 6.0  # comp (ψ = -30°)
    psi_e =  np.pi / 6.0  # ext  (ψ = +30°)

    # Your original Zechstein/Moss Bluff numbers:
