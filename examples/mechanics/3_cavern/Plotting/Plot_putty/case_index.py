...
def filter_cases(cases: list[dict], SELECT: dict) -> list[dict]:
    ...
    def ok(m: dict) -> bool:
        if want_pressure is not None:
            if (m.get("pressure_scenario") or "").lower() != str(want_pressure).lower():
                return False

        if want_scenario is not None:
            if (m.get("scenario_preset") or "").lower() != str(want_scenario).lower():
                return False

        if want_caverns is not None:
            W = [str(x).lower() for x in want_caverns]
            # accept label OR cavern_type OR group-folder (nested)
            if (m.get("cavern_label","").lower() not in W) and (m.get("cavern_type","").lower() not in W) and (m.get("group","").lower() not in W):
                return False

        if want_ncyc is not None and m.get("n_cycles", None) != want_ncyc:
            return False

        if want_days is not None and m.get("operation_days", None) != want_days:
            return False
