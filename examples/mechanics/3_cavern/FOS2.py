...
def compute_FOS_with_lode(p_MPa, q_MPa, sig_Pa_series,
                          q_min_MPa=5,      # drempel waar beneden je FOS vast zet
                          FOS_far=10.0):      # vaste FOS-waarde voor "ver weg van dilatancy"
    """
    p_MPa         : (nt, ncells)
    q_MPa         : (nt, ncells)
    ...
    for it in range(nt):
        ...
        psi_t = compute_lode_angle_from_sigma(sig_t)

        p_t = p_MPa[it, :]                          # (ncells,)
        q_t = q_MPa[it, :]                          # (ncells,)

        q_dil_t = q_dil_rd(p_t, psi_t)              # (ncells,) in MPa

        # masker: waar is q zo klein dat FOS toch niet interessant is?
        mask_far   = (q_t < q_min_MPa)              # boolean (ncells,)
        mask_ratio = ~mask_far

        FOS_t = np.empty_like(q_t)

        # 1) elementen met kleine q → vaste "grote" FOS
        FOS_t[mask_far] = FOS_far

        # 2) elementen met voldoende deviatorische spanning → echte FOS
        #    extra beveiliging tegen deling door 0
        q_use = q_t[mask_ratio]
        q_use = np.where(q_use <= 0.0, q_min_MPa, q_use)
        FOS_t[mask_ratio] = q_dil_t[mask_ratio] / q_use
...
def main():
    base_folder = os.path.join("output", "case_sinus(2)_14days_regular", "operation")
