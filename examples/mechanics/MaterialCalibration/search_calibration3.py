"""Final fine-tuning around best candidates from search2."""
...
DT_HOURS = 0.5
N1_DESAI = 2.0; A1_DESAI = 1e-3; ETA_DESAI = 1.2

# ── FINE GRID around best candidates ──
# Best SIC: eta=1e13, E1=2e9, a0=0.005, mu1=1e-15 (RMSE=0.557, OS=-0.096, drop=-0.065)
# Also:     eta=1e13, E1=1e9, a0=0.003, mu1=3e-15 (RMSE=0.353, OS=+0.821, drop=-0.466)
# Let's explore between E1=1e9 and 3e9

ETA_GRID = [5e12, 8e12, 1e13, 1.5e13, 2e13]
E1_GRID  = [1.0e9, 1.5e9, 2.0e9, 2.5e9, 3.0e9]
A0_GRID  = [0.003, 0.004, 0.005, 0.006, 0.008]
MU1_GRID = [5e-16, 1e-15, 2e-15, 3e-15]

# MD: fine around K0=6e4-7e4
K0_GRID  = [5e4, 6e4, 6.5e4, 7e4, 7.5e4, 8e4]
AW_GRID  = [-14, -12, -10, -8, -6, -4, -2, 0, 2, 4]
...
def main():
    ...
    # ── SIC ──
    print("=" * 130)
    print("SIC FINE-TUNING")
    print("=" * 130)
    
    results = []
    total = len(ETA_GRID)*len(E1_GRID)*len(A0_GRID)*len(MU1_GRID)
    done = 0
    
    for eta_k, E1_k, a0, mu1 in product(ETA_GRID, E1_GRID, A0_GRID, MU1_GRID):
        e_tot = integrate_sic(t_s, sigma_fine_Pa, sigma3_Pa, eta_k, E1_k, a0, mu1)
        rmse, rp1, rp2, rp3, os100, drp, fin = diag(t_mod_h, e_tot, lab_t, lab_e)
        results.append((rmse, eta_k, E1_k, a0, mu1, rp1, rp2, rp3, os100, drp, fin))
        done += 1
        if done % 100 == 0:
    ...
    hdr = f"{'eta_K':>10} {'E1_K':>10} {'a0':>6} {'mu1':>10} | {'RMSE':>6} {'P1':>6} {'P2':>6} {'P3':>6} {'OS100':>7} {'drpErr':>7} {'final':>7}"
    print(hdr); print("-"*130)
    for r in results[:50]:
        rmse, eta_k, E1_k, a0, mu1, rp1, rp2, rp3, os100, drp, fin = r
        print(f"{eta_k:10.2e} {E1_k:10.2e} {a0:6.3f} {mu1:10.2e} | "
              f"{rmse:6.3f} {rp1:6.3f} {rp2:6.3f} {rp3:6.3f} {os100:+7.3f} {drp:+7.3f} {fin:+7.3f}")
    
    # ── MD ──
    print("\n" + "="*100)
    print("MD FINE-TUNING")
    print("="*100)
    md_res = []
    for K0, aw in product(K0_GRID, AW_GRID):
    ...
    print(hdr); print("-"*100)
    for r in md_res[:30]:
        rmse, K0, aw, rp1, rp2, rp3, os100, drp, fin = r
