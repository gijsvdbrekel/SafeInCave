...
def _downsample_xy(x, y, max_points=1500):
    """Downsample (x,y) to at most max_points points, keeping endpoints."""
    x = np.asarray(x)
    y = np.asarray(y)
    n = len(x)
    if n <= max_points:
...
def _rolling_quantile_band(y, window=25, qlo=0.1, qhi=0.9):
    """
    Compute rolling quantile band + rolling median for 1D y.
    Uses a sliding window; robust for oscillatory series.
    """
    y = np.asarray(y)
    n = len(y)
    lo = np.empty(n)
    ...
    for c in cases:
        ...
        try:
            ...
            ax_conv.plot(
                t, conv,
                linewidth=2,
                linestyle=_swing_linestyle(bar),
                color=color,
                alpha=0.9,
                label=f"{bar} bar/day"
            )
        ...
        if t_d is not None:
            t2, p2 = _downsample_xy(t_d, p_mpa, max_points=2000)
            ax_pres.plot(
                t2, p2,
                linewidth=1.2,
                linestyle=_swing_linestyle(bar),
...
def plot_fig_stress_state(cavern_key, cavern_label, cases, swing_colors):
    """
    2x3 figure: 5 probe subplots + pressure schedule.
    ...
    for i, ptype in enumerate(PROBE_ORDER):
        ...
        for bar in sorted(stress_by_swing.keys()):
            ...
            px, qx = _downsample_xy(p, q, max_points=1800)
...
# ══════════════════════════════════════════════════════════════════════════════
# FIGURE 3: FACTOR OF SAFETY (IMPROVED VISUALIZATION)
# ══════════════════════════════════════════════════════════════════════════════
...
def plot_fig_fos(cavern_key, cavern_label, cases, swing_colors):
    ...
    Improvements:
      - Downsample plotted points (prevents dense "yellow wall")
      - Plot rolling P10–P90 band + rolling median (reduces oscillation dominance)
      - Add zoom inset focusing on non-30 swings (makes smaller swings readable)
    """
    fig, axes = plt.subplots(2, 3, figsize=(16, 9))
    axes = axes.flatten()
    ...
    # Compute a sensible zoom range based on non-30 scenarios
    zoom_lo, zoom_hi = None, None
    for bar in sorted(fos_by_swing.keys()):
        if bar == 30:
            continue
        t_days, fos_probes = fos_by_swing[bar]
        for ptype in PROBE_ORDER:
            if ptype in fos_probes:
                y = np.asarray(fos_probes[ptype])
                yclip = np.clip(y, 0.5, 3.0)
                q1, q99 = np.quantile(yclip, [0.01, 0.99])
                zoom_lo = q1 if zoom_lo is None else min(zoom_lo, q1)
                zoom_hi = q99 if zoom_hi is None else max(zoom_hi, q99)

    if zoom_lo is None:
        zoom_lo, zoom_hi = 0.9, 1.4
    else:
        pad = 0.05 * (zoom_hi - zoom_lo + 1e-9)
        zoom_lo = max(0.8, zoom_lo - pad)
        zoom_hi = min(2.0, zoom_hi + pad)
    ...
    for i, ptype in enumerate(PROBE_ORDER):
        ...
        for bar in sorted(fos_by_swing.keys()):
            ...
            tx, fy = _downsample_xy(t_days, fos, max_points=MAX_POINTS)

            if SHOW_BAND:
                lo, med, hi = _rolling_quantile_band(fy, window=BAND_WINDOW, qlo=0.1, qhi=0.9)
                ax.fill_between(tx, lo, hi, color=swing_colors[bar], alpha=0.12, linewidth=0)
                ax.plot(tx, med, linewidth=1.8, linestyle=_swing_linestyle(bar),
                        color=swing_colors[bar], label=f"{bar} bar/day")
                if ADD_INSET:
                    axins.fill_between(tx, lo, hi, color=swing_colors[bar], alpha=0.12, linewidth=0)
                    axins.plot(tx, med, linewidth=1.4, linestyle=_swing_linestyle(bar),
                               color=swing_colors[bar])
            else:
                ax.plot(tx, fy, linewidth=1.6, linestyle=_swing_linestyle(bar),
                        color=swing_colors[bar], alpha=0.9, label=f"{bar} bar/day")
                if ADD_INSET:
                    axins.plot(tx, fy, linewidth=1.2, linestyle=_swing_linestyle(bar),
                               color=swing_colors[bar], alpha=0.9)
        ...
        if ADD_INSET:
            axins.axhline(y=1.0, color='red', linestyle=':', linewidth=1.0, alpha=0.8)
    ...
    if ADD_INSET:
        from mpl_toolkits.axes_grid1.inset_locator import inset_axes
        axins2 = inset_axes(ax_sum, width="40%", height="40%", loc="lower right", borderpad=1.0)
        axins2.grid(True, alpha=0.25)
        axins2.set_ylim(zoom_lo, zoom_hi)
    ...
    for bar in sorted(fos_by_swing.keys()):
        ...
        tx, my = _downsample_xy(t_days, min_fos, max_points=MAX_POINTS)

        if SHOW_BAND:
            lo, med, hi = _rolling_quantile_band(my, window=BAND_WINDOW, qlo=0.1, qhi=0.9)
            ax_sum.fill_between(tx, lo, hi, color=swing_colors[bar], alpha=0.12, linewidth=0)
            ax_sum.plot(tx, med, linewidth=2.0, linestyle=_swing_linestyle(bar),
                        color=swing_colors[bar], label=f"{bar} bar/day")
            if ADD_INSET:
                axins2.fill_between(tx, lo, hi, color=swing_colors[bar], alpha=0.12, linewidth=0)
                axins2.plot(tx, med, linewidth=1.4, linestyle=_swing_linestyle(bar),
                            color=swing_colors[bar])
        else:
            ax_sum.plot(tx, my, linewidth=2.0, linestyle=_swing_linestyle(bar),
                        color=swing_colors[bar], label=f"{bar} bar/day")
    ...
    if ADD_INSET:
        axins2.axhline(y=1.0, color='red', linestyle=':', linewidth=1.0, alpha=0.8)
