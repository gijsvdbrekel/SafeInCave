...
def read_schedule_from_input(output_folder):
    ...
    """
    # 1) Prefer explicit schedule dumped by the solver
    ...
    # 2) Fall back to input_file.json (operation dir or case root)
...
def plot_gas_pressure(ax, time_steps_disp, t_gas, p_gas, t_step):
    """ Plot schema + marker op huidige displacement-tijdstap. """
    ax.plot(t_gas/day, p_gas/MPa, "-", color="black", linewidth=1.5)
    # marker op huidige tijd van de verplaatsingsresultaten
...
def main():
	...
	# Drukschema uit input_file.json
	...
	def update_plot(val):
		...
		for i, ax in enumerate([ax00, ax01, ax02, ax10, ax11, ax12]):
			...
			# Gas pressure updaten
		xmin, xmax = ax30.get_xlim()
		ymin, ymax = ax30.get_ylim()
		ax30.cla()
		plot_gas_pressure(ax30, wall_profile.time_list, t_gas, p_gas, t_step=t_step)
