...
def tile_profile(times_s, pressures_pa, weeks: int):
	...
	# sort + deduplicate exact equal times
...
def safety_check_24h(time_list_s, p_list_pa, limit_bar=10.0):
	...
	return max_drop / 1e5  # return in bar
...
# ---------- Custom momentum class to expose internal fields ----------
...
def main():
	...
	# ---------- Solver (keep your cg+asm, or switch if you prefer) ----------
	...
	# ---------- Initial T (kept constant; no geothermal modeling here) ----------
	...
	# Neumann BCs (lithostatic/top)
	side_burden = 10.0 * MPa
	over_burden = 10.0 * MPa
	gas_density = 0.082
	p_gas_eq    = 10.0 * MPa

	bc_east = momBC.NeumannBC("East",  direction=2, density=salt_density, ref_pos=660.0,
							  values=[side_burden, side_burden],
							  time_values=[0.0, tc_equilibrium.t_final], g=g_vec[2])
	bc_north = momBC.NeumannBC("North", direction=2, density=salt_density, ref_pos=660.0,
							   values=[side_burden, side_burden],
							   time_values=[0.0, tc_equilibrium.t_final], g=g_vec[2])
	bc_top = momBC.NeumannBC("Top", direction=2, density=0.0, ref_pos=0.0,
							 values=[over_burden, over_burden],
							 time_values=[0.0, tc_equilibrium.t_final], g=g_vec[2])
	bc_cavern_eq = momBC.NeumannBC("Cavern", direction=2, density=gas_density, ref_pos=430.0,
	...
	# Reference pressure (like your v1 roof scale; here we keep 10 MPa for clarity)
	p_ref = 10.0 * MPa

	# ---- Choose scenario ----
	scenario = "chemical"   # options: "baseline", "chemical", "power", "weekly_weekday_draw", "weekly_twice"

	# ---- Build (time_list [s], p_vals [Pa]) ----
	if scenario == "baseline":
		...
		knots_days = [0, 3, 5, 7]        # Mon00, Thu00, Sat00, next Mon00
	...
	# ---- Time controller for operation (final_time in hours) ----
	tc_operation = sf.TimeController(
		dt=0.1, initial_time=0.0,
	...
	# Cavern pressure with scenario (values in Pa, time_values in s)
	...
	# Optional: write pressure schedule for plotter
	if MPI.COMM_WORLD.rank == 0:
		sched = {"time_list": list(map(float, time_list)),
				 "pressure":  list(map(float, p_vals))}
		os.makedirs(output_folder_operation, exist_ok=True)
		with open(os.path.join(output_folder_operation, "pressure_schedule.json"), "w") as f:
			json.dump(sched, f)
