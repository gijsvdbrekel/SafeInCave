...
def load_cavern_pressure(output_folder):
    file_path = os.path.join(output_folder, "bc_cavern", "cavern_pressure.csv")
    data = np.genfromtxt(file_path, delimiter=",", skip_header=1)
    time_day = data[:,0]
    pressure_MPa = data[:,1]
    return time_day, pressure_MPa
...
def plot_gas_pressure(ax, time_day, pressure_MPa, t_step):
    ax.plot(time_day, pressure_MPa, "-", color="black")

    # t_step = min(t_step, len(time_day) - 1)
    t_step = min(int(t_step*0.5/2), len(time_day) - 1)
    ax.scatter(time_day[t_step], pressure_MPa[t_step],
               c="white", edgecolors="black", zorder=10000)

    ax.set_xlabel("Time (days)", size=12, fontname="serif")
    ax.set_ylabel("Gas pressure (MPa)", size=12, fontname="serif")
...
def main():
    ...
    output_folder = os.path.join("output", "case_1", "operation")
    # output_folder = os.path.join("output", "case_gui", "operation")

    # Geometry-based probe locations
    R = 25
    cave_height = 130
    ovb_width = 700
    roof_width = 200
    ...
    x3, z3 = R,      z1 - R - (cave_height-2*R)/4
    x4, z4 = R,      z1 - cave_height + R + (cave_height-2*R)/4
    ...
    time_day, pressure_MPa = load_cavern_pressure(output_folder)
    ...
    plot_gas_pressure(ax30, time_day, pressure_MPa, t_step=0)
    ...
    def update_plot(val):
        ...
        ax0.cla()
        plot_cavern_shape(ax0, wall_profile, t_step=t_step)
        plot_probes(ax0, probes)
        ...
        ax30.cla()
        plot_gas_pressure(ax30, time_day, pressure_MPa, t_step=t_step)
