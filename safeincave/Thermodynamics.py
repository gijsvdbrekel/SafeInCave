...
class CavernThermodynamics(object):
    ...
    def solve_injection(
        ...
        *,
        ...
        logPin_guess = np.log(Pin_guess)

        x0 = np.array([logP1_guess, T1_guess, logPin_guess], dtype=float)

        rho_target = M1 / V1

        # Inlet enthalpy
        rho_in, _, h_in = self.rho_u_h(Pin_guess, Tin)
        ...
        density_scale = max(1.0, abs(rho_target))
        flow_scale = max(1.0, abs(2*Pin_guess))

        def residuals(x: np.ndarray) -> np.ndarray:
            logP1, T1, logPin = float(x[0]), float(x[1]), float(x[2])
            P1 = np.exp(logP1)
            Pin = np.exp(logPin)
            ...
            try:
                rho_in, _, h_in = self.rho_u_h(Pin, Tin)
            except Exception as e:
                # Return large residuals if CoolProp fails at this state
                return np.array([1e20, 1e20, 1e20], dtype=float)
            ...
            return np.array([F1/energy_scale, F2/density_scale, F3/flow_scale], dtype=float)
